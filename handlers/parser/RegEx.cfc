<cfcomponent displayname="javaRegExp" output="false" hint="A collection of UDFs that act as wrappers around native Java regular rxpressions from the java.util.regex package. They use Java syntax for regular expressions; compared to CFML native functions they also support lookbehind and inline modifiers. For additional info check Sun documentation and the following books: http://www.oreilly.com/catalog/regex2/, http://www.oreilly.com/catalog/regexppr/ It is strongly suggested to only use the following methods: Test(), Replace(), Matches and Exec().  Use the other methods only if you are familiar with the java.util.regex package or need to work at a lower level in order to optimise performances. Require ColdFusion MX 6.1+ or JRE 1.4+">
	...
	* A collection of UDFs that act as wrappers around native Java regular rxpressions from the java.util.regex package.
	  ...
	  It is strongly suggested to only use the following methods: RegTest(), RRegeplace(), RegMatches and RegExec().
	  ...
	  Require ColdFusion MX 6.1+ or JRE 1.4+
	* @author      Massimo Foti (massimo@massimocorner.com)
	* @version     1.1, 12/23/2003
	 */
	  --->
	
	<cffunction access="public" name="init" output="false" hint="Constructor" returntype="any">
		<cfreturn this />
	</cffunction>
	...
	* @return      array
	 */
	  --->
		<cffunction access="public" name="jMatcherExec" output="false" returntype="array" hint="Search the target string attached to the matcher and returns the results as an array, where the first element contains the whole match and any subsequent element contains matches from capturing groups">
	 ...
	 */
	  --->
		<cffunction access="public" name="jMatcherMatches" output="false" returntype="array" hint="Search a string using a regular expression and returns all matching subsequences as an array">
			<cfargument name="jMatcher" required="yes" hint="The Java Pattern object. Call jMatcherInstance() to create one">
			<cfargument name="returnStruct" type="boolean" required="no" default="false" hint="By default matches are returned as strings, set this to true in order to get a structure that contains three elements: string (matched string), start (index of the first character matched), end (index of the last character matched), sub (array of matches from capturing groups)">
			<cfset var i=0>
			<cfset var matchesArray=ArrayNew(1)>
			<cfset var matchStruct="">
			<cfset validateMatcher(arguments.jMatcher)>
			...
			<cfscript>
				while(jMatcher.find()){
					if(NOT arguments.returnStruct){
						...
						for(i=1; i <= jMatcher.groupCount(); i++){
	...
	* @return      array
	 */
	  --->
		<cffunction access="public" name="RegExec" output="false" returntype="array" hint="Search a string using a regular expression and returns the results as an array, where the first element contains the whole match and any subsequent element contains matches from capturing groups">
			<cfargument name="regExp" type="string" required="yes" hint="The regular expression string literal">
			<cfargument name="targetString" type="string" required="yes" hint="The target string">
			<cfargument name="returnStruct" type="boolean" required="no" default="false" hint="By default matches are returned as strings, set this to true in order to get a structure that contains three elements: string (matched string), start (index of the first character matched), end (index of the last character matched)">
	...
	* @return      array
	 */
	  --->
		<cffunction access="public" name="RegMatches" output="false" returntype="array" hint="Search a string using a regular expression and returns all matching subsequences as an array">
			<cfargument name="regExp" type="string" required="yes" hint="The regular expression string literal">
			<cfargument name="targetString" type="string" required="yes" hint="The target string">
			<cfargument name="returnStruct" type="boolean" required="no" default="false" hint="By default matches are returned as strings, set this to true in order to get a structure that contains three elements: string (matched string), start (index of the first character matched), end (index of the last character matched), sub (array of matches from capturing groups)">
			<cfset var jMatcher=jMatcherInstance(arguments.regExp, arguments.targetString)>
	...
	* @return      string
	 */
	  --->
	  <cffunction access="public" name="RegReplace" output="false" returntype="string" hint="Searches a string using a regular expression and replace matching subsequences with the given replacement string">
	    <cfargument name="regExp" type="string" required="yes" hint="The regular expression string literal">
	    <cfargument name="targetString" type="string" required="yes" hint="The target string">
	    <cfargument name="replacement" type="string" required="yes" hint="The replacement string">
	    <cfargument name="all" type="boolean" required="no" default="true" hint="By default all matches are replaced, set this to false to replace only first subsequence">
	...
	* @return      boolean
	 */
	  --->
	  <cffunction access="public" name="RegTest" output="false" returntype="boolean" hint="Returns a boolean to indicate whether the regular expression matches anything in the target string">
	    <cfargument name="regExp" type="string" required="yes" hint="The regular expression string literal">
	    <cfargument name="targetString" type="string" required="yes" hint="The target string">
